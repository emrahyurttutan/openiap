...
import { IAPKIT_MCP_LOOPBACK_HEADER, kitClient, normalizeKitBaseUrl } from '../src/kit-client';
...
describe('normalizeKitBaseUrl', () => {
  it('normalizes http(s) base URLs without credentials', () => {
    expect(normalizeKitBaseUrl('https://kit.example/')).toBe('https://kit.example');
    expect(normalizeKitBaseUrl('http://localhost:3000')).toBe('http://localhost:3000');
  });
...
describe('kitClient', () => {
  it('marks only loopback API calls as internal MCP traffic', async () => {
    ...
    expect(fetchMock.mock.calls[1]?.[1]?.headers).not.toHaveProperty(IAPKIT_MCP_LOOPBACK_HEADER);
  });
  ...
  it('includes the full kit path in HTTP error messages', async () => {
    ...
    await expect(client.listProducts()).rejects.toThrow('kit /v1/products returned 403');
  });
});
