...
import { createIapKitMcpServer, IAPKIT_MCP_SERVER_NAME, IAPKIT_MCP_SERVER_VERSION } from './mcp.js';
...
export function createRemoteMcpHttpServer(
  ...
  const transports = createMcpSessionStore<StreamableHTTPServerTransport>(logger);
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST') {
        await handleMcpPost(req as AuthenticatedRequest, res, transports, logger, machineId);
        return;
      }
...
async function handleMcpPost(
  ...
  if (!reservation) {
    res.setHeader('Retry-After', String(MCP_SESSION_CAPACITY_RETRY_AFTER_SECONDS));
...
function writeUnknownSessionResponse(
  ...
  writeJsonRpcError(res, 404, -32001, 'Session not found — initialize a new MCP session.');
}
...
function setCorsHeaders(req: IncomingMessage, res: ServerResponse, allowedOrigins: string[]): void {
  ...
  const allowOrigin = origin && (allowAll || allowedOrigins.includes(origin)) ? origin : null;
